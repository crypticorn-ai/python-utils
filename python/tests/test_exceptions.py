...
from fastapi import FastAPI, HTTPException, WebSocketException, Request
...
from crypticorn_utils.errors import ErrorType, ErrorLevel
...
class TestExceptionHandler:
    ...
    def test_exception_handler_websocket_initialization(self, mock_error_callback):
        ...
        handler = ExceptionHandler(callback=mock_error_callback, type=ExceptionType.WEBSOCKET)
        assert handler.callback == mock_error_callback
    ...
    def test_build_exception_with_message(self, exception_handler):
        ...
        exception = exception_handler.build_exception("test_error", message="Custom message")
...
class TestExceptionHandlerMethods:
    ...
    async def test_request_validation_handler(self, mock_request):
        ...
        response = await ExceptionHandler._request_validation_handler(mock_request, validation_error)
    ...
    async def test_response_validation_handler(self, mock_request):
        ...
        response = await ExceptionHandler._response_validation_handler(mock_request, validation_error)
